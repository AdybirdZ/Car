...
static void Encoder_New_Left_Callback (uint32 event, void *ptr)
{
    (void)event;
    (void)ptr;
    encoder_new_count[ENCODER_NEW_LEFT] ++;
}
...
static void Encoder_New_Right_Callback (uint32 event, void *ptr)
{
    (void)event;
    (void)ptr;
    encoder_new_count[ENCODER_NEW_RIGHT] ++;
}
...
static float Encoder_New_Count_To_Speed (uint32 count, uint8 motor)
{
    ...
    // 速度 PID 只需每个采样周期的 A 相脉冲数。B 相已保留为 GPIO 输入，供后续增加反转计数功能。
    (void)motor;
    return speed;
}
...
{
    encoder_new_count[ENCODER_NEW_LEFT] = 0;
    encoder_new_count[ENCODER_NEW_RIGHT] = 0;
    encoder_new_speed[ENCODER_NEW_LEFT] = 0.0f;
    encoder_new_speed[ENCODER_NEW_RIGHT] = 0.0f;
...
{
    uint32 left_count = encoder_new_count[ENCODER_NEW_LEFT];
    uint32 right_count = encoder_new_count[ENCODER_NEW_RIGHT];

    encoder_new_count[ENCODER_NEW_LEFT] = 0;
    encoder_new_count[ENCODER_NEW_RIGHT] = 0;

    encoder_new_speed[ENCODER_NEW_LEFT] = Encoder_New_Count_To_Speed(left_count, ENCODER_NEW_LEFT);
    encoder_new_speed[ENCODER_NEW_RIGHT] = Encoder_New_Count_To_Speed(right_count, ENCODER_NEW_RIGHT);
}
